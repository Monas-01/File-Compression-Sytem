public class HuffmanEncoder {
    ...
    // Note: '\0' dummy nodes inserted for single-distinct-char input are skipped.
    public void lookUp(Node node, String s) {
        if(!node.isLeaf()) {
            lookUp(node.left, s + "0");
            lookUp(node.right, s + "1");
        } else if(node.character != '\0') {
            // Assign at least "0" so single-char messages still get a valid code
            charset.add(node.character, s.isEmpty() ? "0" : s, node.frequency);
        }
    }
